...
df_rendemand_longer <- df_renewablegen_demand %>%pivot_longer(cols=c('halfhourlysolar','halfhourlywind','halfhourlydemand'), names_to='source', values_to='value')

df_renewablegen_demand %>% ggplot(aes(x = SETTLEMENT_PERIOD)) + 
  geom_line(aes(y = halfhourlysolar), color ='yellow', size=1) +
  geom_line(aes(y=halfhourlywind), color='blue' ,size=1) +
  geom_line(aes(y=halfhourlydemand), color='red')
...
set.seed(23)
df <- read_csv("C:/Users/k_gag/OneDrive/Desktop/Dissertation_V2G/results/30_V2G/30_all.csv")


# Create random forest for regression
df.rf <- randomForest(Available ~ ., data = df, mtry = 3,
